...
vector<int> solution(vector<int> sequence, int k) {
    ...
    {
        if(!answer.empty() && distance(startP,endP) > (answer[1]-answer[0]+1))
        {
            return answer;
        }
    	...
    	else if(CurrentSum == k)
        {
            ...
            if(answer.size()!=0)
            {
                ...
                {
                    answer.clear();
                    answer.push_back(fwd);
                    answer.push_back(bwd);
                    p = startP;
                    continue;
                }
                else
    		...
    		else
            {
                ...
                answer.push_back(sequence.size() - distance(sequence.rbegin(), endP) - 1);
                answer.push_back(sequence.size() - distance(sequence.rbegin(), startP) - 1);
            }
        }
    }
